/**
 * LogShipper - Centralized log shipping service for gsuite
...
export class LogShipper {
  ...
  private validateAndCleanLogs(logs: LogEntry[]): LogEntry[] {
    return logs.map(log => {
      ...
      const cleanedLog: LogEntry = {
        ...
        serverName: log.serverName || 'gsuite',
  ...
  private generateSessionId(): string {
    return `gsuite-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
  }
  ...
  private logLocally(level: string, message: string, metadata?: any): void {
    ...
    console.error(`[GSUITE-LOG-SHIPPER] ${JSON.stringify(logEntry)}`);
  }
